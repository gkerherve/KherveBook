...
BOIDS_SOURCE = (
    ...
    "    birds_pos = _rng.uniform(0.05, 0.95, (120, 2))\n"
    "    _a = _rng.uniform(0, 2 * np.pi, 120)\n"
    "    birds_vel = 0.013 * np.c_[np.cos(_a), np.sin(_a)]\n"
    "    birds_goal = np.array([0.5, 0.5])   # a target the flock roams to\n"
    "    birds_gvel = np.array([0.006, 0.004])\n"
    "_N = len(birds_pos)\n"
    "birds_goal = birds_goal + birds_gvel               # drift the goal…\n"
    "birds_gvel = np.where((birds_goal < 0.12) | (birds_goal > 0.88),\n"
    "                      -birds_gvel, birds_gvel)      # …bounce off walls\n"
    "birds_goal = np.clip(birds_goal, 0.12, 0.88)\n"
    ...
    "birds_vel += 0.006 * (birds_goal - birds_pos)   # chase the roaming goal\n"
    "birds_vel += 0.003 * (birds_pos < 0.08)         # veer off the walls…\n"
    "birds_vel -= 0.003 * (birds_pos > 0.92)         # …before they hit them\n"
    "birds_vel += 0.0011 * _rng.standard_normal((_N, 2))   # a little wander\n"
    "_sp = np.hypot(*birds_vel.T)[:, None]\n"
    "birds_vel *= np.clip(_sp, 0.010, 0.017) / _sp   # keep them flying\n"
    "birds_pos = np.clip(birds_pos + birds_vel, 0.02, 0.98)\n"
    "_u = birds_vel / np.hypot(*birds_vel.T)[:, None]   # unit headings\n"
    "_w = np.c_[-_u[:, 1], _u[:, 0]]                    # wing direction\n"
    "_tri = np.stack([birds_pos + 0.026 * _u,           # beak\n"
    "                 birds_pos - 0.016 * _u + 0.011 * _w,\n"
    "                 birds_pos - 0.016 * _u - 0.011 * _w], axis=1)\n"
...
def _boids():
    return [
        _md("# Flocking Birds\nA boids flock — separation, alignment and "
            "cohesion, plus a roaming goal and a touch of wander, send the "
            "triangles wheeling across the canvas as one. Stop with the "
            "red button."),
        _code(BOIDS_SOURCE),
    ]
