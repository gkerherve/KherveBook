...
Each ``.kbook`` lives in a directory that is also its own git repo. The
committed artifact is the human-readable notebook file itself (``.kbook``
...
def _signature(repo: "pygit2.Repository | None" = None) -> "pygit2.Signature":
    ...
    return pygit2.Signature(
        "KherveBook", "khervebook@local", int(datetime.now().timestamp()), 0)
...
def pull(repo_dir: Path, remote_name: str = "origin") -> tuple[bool, str]:
    ...
    needed the user should drop to the command line. The KherveBook
    ...
    if not (merge_analysis & pygit2.GIT_MERGE_ANALYSIS_FASTFORWARD):
        return False, (
            "Local branch has diverged from the remote — KherveBook only "
...
def history_detailed(repo_dir: Path,
    ...
    When *file_stem* is given (e.g. ``"Book1"``), only commits that
